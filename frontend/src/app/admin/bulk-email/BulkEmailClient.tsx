...
export default function BulkEmailClient() {
  ...
  return (
    <div className="bg-brand-white border border-brand-primary/20/50 rounded-xl p-8 max-w-4xl">
      <div className="mb-8">
        <h3 className="text-2xl font-bold text-brand-text">Bulk Email Campaigns</h3>
      ...
      <form id="bulk-email-form" action={handleSend} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-brand-text/80 mb-2">Subject Line</label>
          <input 
            type="text" 
            name="subject" 
            required
        ...
        <div>
          <label className="block text-sm font-medium text-brand-text/80 mb-2">Email Body (HTML supported)</label>
          <textarea 
            name="body" 
            required
            rows={12}
        ...
        <div className="pt-4 border-t border-brand-primary/20/30 flex justify-end">
          <button 
            type="submit" 
            disabled={isPending}
            className="px-8 py-3 bg-brand-primary text-brand-white font-bold rounded-lg hover:bg-blue-700 active:scale-95 transition-all disabled:opacity-50 flex items-center gap-2"
          >
            {isPending ? (
              <>
