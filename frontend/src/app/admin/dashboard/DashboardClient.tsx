...
export default function DashboardClient({ 
  ...
  return (
    <div className="bg-[#031427] text-[#d3e4fe] min-h-screen font-sans selection:bg-blue-600/30 selection:text-white">
      ...
      {/* Sidebar Navigation */}
      <aside className={`hidden lg:flex flex-col h-screen fixed left-0 top-0 bg-[#0b1c30] border-r border-[#424656] shadow-sm z-50 transition-all duration-300 ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
        ...
        <div className="p-4 border-t border-[#424656]/30 space-y-4 mt-auto">
          <button 
            ...
            className={`w-full flex items-center ${isSidebarCollapsed ? 'justify-center' : 'justify-between px-4'} py-2 text-[#8c90a1] hover:text-[#d3e4fe] transition-colors mb-2`}
          ...
          <button 
            ...
            className={`w-full bg-blue-600/10 text-blue-400 border border-blue-600/20 text-xs font-bold py-3 rounded-lg hover:bg-blue-600 hover:text-white active:scale-[0.98] transition-all flex items-center justify-center gap-2 ${isSidebarCollapsed ? 'px-0' : 'px-4'}`}
            title="Invite Author"
          >
      ...
      <main className={`transition-all duration-300 min-h-screen flex flex-col ${isSidebarCollapsed ? 'lg:ml-20' : 'lg:ml-64'}`}>
            ...
            <div className="relative">
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-3 w-56 glass-panel border border-[#424656]/50 rounded-xl shadow-2xl py-2 z-50 transform origin-top-right transition-all">
                  ...
                  <div className="my-2 border-t border-[#424656]/30"></div>
        ...
        <div className="p-6 md:p-10 space-y-10 max-w-[1600px] mx-auto w-full">
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div>
              <h2 className="text-4xl font-semibold text-[#d3e4fe]">{overrideTitle || "Dashboard"}</h2>
              <p className="text-[#c2c6d8] text-base mt-2">{overrideSubtitle || "Welcome back. Here's what's happening in the academy today."}</p>
            </div>
            ...
            <>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="glass-panel p-6 rounded-xl group hover:border-[#b3c5ff]/50 transition-all duration-500">
                  ...
                  <h3 className="text-3xl font-semibold text-[#d3e4fe] mt-2 transition-colors">{stats?.total || 0}</h3>
                ...
                <div className="glass-panel p-6 rounded-xl group hover:border-[#ffb59d]/50 transition-all duration-500">
                  ...
                  <h3 className="text-3xl font-semibold text-[#d3e4fe] mt-2">{stats?.pending || 0}</h3>
                  <div className="mt-6 h-12 w-full overflow-hidden">
                ...
                <div className="glass-panel p-6 rounded-xl group hover:border-[#bec6e0]/50 transition-all duration-500">
                  ...
                  <h3 className="text-3xl font-semibold text-[#d3e4fe] mt-2">{stats?.approved || 0}</h3>
                  <div className="mt-6 h-12 w-full overflow-hidden">
                    <svg className="w-full h-full" viewBox="0 0 100 40">
                ...
                <div className="glass-panel p-6 rounded-xl group hover:border-blue-600/50 transition-all duration-500">
                  ...
                  <h3 className="text-3xl font-semibold text-[#d3e4fe] mt-2">{stats?.published || 0}</h3>
                  <div className="mt-6 h-12 w-full overflow-hidden">
                    <svg className="w-full h-full" viewBox="0 0 100 40">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-12 glass-panel rounded-xl flex flex-col overflow-hidden">
                  <div className="p-6 border-b border-[#424656]/30 flex justify-between items-center">
                    <h4 className="text-2xl font-semibold text-[#d3e4fe]">Recent Submissions</h4>
                    <Link href="/admin/authors" className="text-[#b3c5ff] text-xs font-medium hover:underline">View All</Link>
                  </div>
                      ...
                      <tbody className="divide-y divide-[#424656]/20">
                        {authors.length === 0 ? (
                          ...
                          <tr key={author.id} className="hover:bg-[#102034]/30 transition-colors">
                            ...
                            <td className="px-6 py-4 text-sm text-[#c2c6d8]">{author.bookTitle || '-'}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      {inviteModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-[#0b1c30] border border-[#424656] rounded-xl p-8 max-w-md w-full shadow-2xl relative">
            <button 
              onClick={() => setInviteModalOpen(false)}
              className="absolute top-4 right-4 text-[#8c90a1] hover:text-[#d3e4fe] transition-colors"
            >
              <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg>
            </button>

            <h3 className="text-xl font-bold text-[#d3e4fe] mb-2">Quick Invite Author</h3>
            <p className="text-sm text-[#8c90a1] mb-6">Send an exclusive invite link directly to an author's email.</p>
            
            <form onSubmit={handleInvite} className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-[#c2c6d8] mb-1">Author Name (Optional)</label>
                <input 
                  type="text" 
                  value={inviteName}
                  onChange={(e) => setInviteName(e.target.value)}
                  className="w-full bg-[#102034] border border-[#424656] text-[#d3e4fe] rounded-lg p-3 text-sm focus:outline-none focus:border-blue-500" 
                  placeholder="John Doe" 
                />
              </div>
              <div>
                <label className="block text-xs font-medium text-[#c2c6d8] mb-1">Email Address *</label>
                <input 
                  type="email" 
                  required
                  value={inviteEmail}
